  ...
  void initState() {
    ...
    _capTimer = Timer.periodic(const Duration(minutes: 1), (_) {
      ...
      if (running && _resumedAt != null) {
        ...
        if (runFor >= _kHardCap) {
          _pauseWithSnack(
              '5時間を超えたため一時停止しました', withResume: true);
          _resumedAt = null;
        }
      }
    ...
    setState(() {
      ...
      if (section.selectedPart != null) {
        ...
        for (final name in names) {
          ...
          if (isAerobic) {
            final String dist = (rec?.distance
                ?.trim()
                .isNotEmpty ?? false)
                ? rec!.distance!.trim()
                : (lu?.distance?.trim() ?? '');
            final String dura = (rec?.duration
                ?.trim()
                .isNotEmpty ?? false)
                ? rec!.duration!.trim()
                : (lu?.duration?.trim() ?? '');
          ...
          AlertDialog(
            ...
            actions: [
              ...
              TextButton(
                  ...
                  child: Text(
                      l10n.delete, style: const TextStyle(color: Colors.red))),
            ],
          ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    final dial = Positioned(
      ...
      child: _fabOpen
        ...
        children: [
          chipAction(
              l10n.addSet, () => _handleAddSet(l10n), enabled: canAddSet()),
          const SizedBox(height: 8),
      ...
      child: PopScope(
        ...
        child: Scaffold(
          ...
          appBar: AppBar(
            ...
            // ← タイトルも白字
            title: const Text(
              // formattedDate を使うなら → Text(formattedDate, style: TextStyle(...)) に戻してください
              '記録',
              style: TextStyle(
                color: Colors.white,
                fontWeight: FontWeight.bold,
            ...
            // ← 壁紙を活かすため、個別の背景色指定は削除
            // backgroundColor: colorScheme.surface,

            // ← 上部にぼかし＋半透明グラデを敷いてどんな壁紙でも文字が読めるように
          ...
          Padding(
            ...
            child: widget.isAerobic
              ...
              children: [
                    ...
                    Expanded(
                      ...
                      child: Focus(
                        ...
                        child: StylishInput(
                          ...
                          suggestionTextColor: colorScheme.onSurfaceVariant
                              .withValues(alpha: 0.5),
                          fillColor: colorScheme.surfaceContainer,
                ...
                // 時間（min/sec はタップで picker。StylishInput は AbsorbPointer で非編集）
                Row(
                  children: [
                    Text(
                      l10n.time,
